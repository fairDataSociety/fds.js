...
module.exports = function (deployer, network, accounts) {
  return deployer.then(async () => {
    if (networks.includes(network)) {
      ...
      await ens.setSubnodeOwner(namehash.hash('eth'), '0x' + sha3('fairdrop'), accounts[0])

      const resolver = await PublicResolver.deployed()
      await ens.setResolver(namehash.hash('fairdrop.eth'), resolver.address)

      const dhr = await TestRegistrar.deployed()
      await ens.setSubnodeOwner('0x0', '0x' + sha3('eth'), accounts[0])

      await deployer.deploy(SubdomainRegistrar, ens.address, namehash.hash('fairdrop.eth'))

      const registrar = await SubdomainRegistrar.deployed()
      await ens.setSubnodeOwner(namehash.hash('eth'), '0x' + sha3('fairdrop'), registrar.address)
    } else {
