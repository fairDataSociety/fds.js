...
class FDSENS2 {
    ...
    setResolver(subdomain, confirm = false){
      let node = hash(subdomain + '.' + this.config.domain);
    ...
    setAll(subdomain, content, multihash, name, confirm = false) {
        ...
        return this.PublicResolver.setAll(node, "0x"+this.account.wallet.address, content, multihash, publicKeyX, publicKeyY, name, confirm);
    }
