...
class Accounts {
    ...
    constructor(config) {
        ...
        this.Swarm = new Swarm(config, this);
                ...
                return this.ensureHasBalance(address).then((balance) => {
                    ...
                    return this.web3.eth.getTransactionCount(wallet.wallet.getAddressString()).then((nonce) => {
                        ...
                        this.ENS.setPubKey(subdomain, wallet, nonce).then((response) => {
                            feedbackMessageCallback('deploying multibox contract...');
                            return response;
                        });
                        nonce = nonce + 1;
                        //return this.Mailbox.deployMailbox(wallet, nonce).then((response) => {
                        return this.Multibox.deployMultibox(wallet, nonce).then((response) => {
                            feedbackMessageCallback('multibox contract deployed...');

                            nonce = nonce + 1;
                            this.Multibox.initialize(wallet, response.contractAddress, nonce).then((responseM) => {
                                feedbackMessageCallback('initialize multibox...');
                                return responseM;
                            });
