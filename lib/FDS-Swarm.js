...
class Swarm {
  ...
  async getSwarmDigestValue(swarmhash) {
    ...
    switch (swarmhash) {
      ...
      default:
        let value;
        try {
          value = await this.bee
            .downloadData(swarmhash)
            .then((data) => data.text());
        } catch (e) {
          throw new Error(e);
        }
  ...
  storeFilesUnencrypted(
    ...
    //   size += files[i].size;
    //   let fileName = files[i].fullPath
    //     ? files[i].fullPath.split('/').splice(1).join('/')
    //     : files[i].name;
  ...
  storeEncryptedFile(
    ...
    return Utils.fileToBuffer(file).then((buffer) => {
      ...
      return Crypto.encryptBuffer(buffer, secret, iv).then(
        (encryptedBuffer) => {
  ...
  getDecryptedFile(hash, secret, selectedMailbox, selectedWallet) {
    return this.getDataFromManifest(hash.address, hash.file.name).then(
      (retrievedFile) => {
        let decryptedBuffer = Crypto.decryptBuffer(
          retrievedFile,
          secret,
          Buffer.from(hash.iv.substring(2, 34), 'hex')
        );
