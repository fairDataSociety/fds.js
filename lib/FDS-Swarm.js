...
class Swarm {
    ...
    storeFileUnencrypted(file, uploadedProgressCallback = console.log, progressMessageCallback = console.log) {
            let url = this.gateway + '/bzz:/' + file;
            var formData = new FormData();
            formData.append('file', file);
            return this.sendRequest(url, 'POST', 'text', formData, uploadedProgressCallback).then((hash) => {
                return new Hash({
                    address: hash,
                    file: file,
                    time: Date.now()
    ...
    getDecryptedFile(hash, secret, selectedMailbox, selectedWallet) {
        return this.getDataFromManifest(hash.address, hash.file.name).then((retrievedFile) => {
            ...
            let decryptedBuffer = Crypto.decryptBuffer(retrievedFile, secret, hash.iv);
    ...
    postData(data, progressCallback = console.log, protocol = 'bzz:/') {
        return this.sendRequest(this.gateway + '/' + protocol, 'POST', 'text', data, progressCallback);
    }
     ...
     */
    sendRequest(url, requestType, responseType, data, progressCallback) {
        console.time('sendRequest//Swarm');
        return axios.request({
            ...
            console.timeEnd('sendRequest//Swarm');
    ...
    getDataFromManifest(swarmHash, filename) {
        ...
        return this.getFile(url).then((manifest) => { // TODO remove double JSON parse
            ...
            if (JSON.parse(manifest).entries.length === 1) {
                return this.getData(this.rawGateway + JSON.parse(manifest).entries[0].hash + "/").then((data)=>{
                    return data;
                });
            } else {
