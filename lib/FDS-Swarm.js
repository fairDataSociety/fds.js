...
class Swarm {
    ...
    storeFileUnencrypted(file, uploadedProgressCallback = console.log, progressMessageCallback = console.log) {
            ...
            return this.sendRequest(url, 'POST', 'text', formData, uploadedProgressCallback, file.size).then((hash) => {
                return new Hash({
                    address: hash,
                    file: file,
                    time: Date.now()
    ...
    */
    ...
    postData(data, progressCallback = console.log, protocol = 'bzz:/') {
        return this.sendRequest(this.gateway + '/' + protocol, 'POST', 'text', data, progressCallback, data.length);
    }
