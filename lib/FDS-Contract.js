...
class FDSContract {
    ...
    deploy(args = [], gas = 15000000) {
      console.time('deployTransaction//Contract');  
      return new Promise((resolve, reject)=>{
        ...
        // console.log('txdeploy', this.account.nonce, args, tx);
        ...
        return this.Web3.eth.accounts.signTransaction(tx, privateKey).then((signed) => {
            return this.Web3.eth.sendSignedTransaction(signed.rawTransaction)
                .once('confirmation', (i, receipt) => {
                  console.timeEnd('deployTransaction//Contract');                      
    ...
    send(method, args = [], confirm = true, gas = 15000000) {
      console.time('sendTransaction//Contract');
      return new Promise((resolve, reject)=>{
        ...
        // console.log('txsend', this.account.nonce, args, tx);

        return this.Web3.eth.accounts.signTransaction(tx, privateKey).then((signed) => {
            return this.Web3.eth.sendSignedTransaction(signed.rawTransaction)
                .once('transactionHash', function (hash) {
                    if(confirm === false){
                      console.log('sendTransaction//Contract', method, confirm);
                      console.timeEnd('sendTransaction//Contract');                                          
                      resolve(hash);
                    }
                })
    ...
    call(method, args = [], gas = 15000000) {
      ...
      let response =  web3Instance.methods[method](...args).call({from: this.account.address});

      console.log('call//Contract', method);
      console.timeEnd('call//Contract');   
