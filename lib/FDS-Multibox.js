...
class Multibox {
    ...
    // multiboxe has to be initialized
     ...
     * @returns {TransactionReceipt} transactio receipt
     */
    async newRequest(senderAccount, recipientSubdomain, multiboxAddress, feedLocationHash) {
    ...
    getRequests(multiboxAddress) {
        let recipientMultiboxContract = new this.web3.eth.Contract(MultiboxContract.abi, multiboxAddress);
        let requests = recipientMultiboxContract.methods.getRequests().call();
    ...
    async traverseKVTNodes(account, kvtContract, node, parentNodeData) {
        ...
        for (let i = 0; i < children.length; i++) {
            //let childData = await this.addKVTNode(kvtContract, rootNode, childData);
            //console.log(node + " traverse:", children[i]);
    ...
    async getKvt(multiboxContractAddress, index) {
        let multiboxContract = await this.getMultiboxContract(multiboxContractAddress);
        let roots = await this.getRoots(multiboxContract);
        let kvtContract = await this.getKvtContract(roots[index]); 
        return kvtContract;
    }
    ...
    async createPath(acc, fullpath, multiboxContractAddress, root=0)
    {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (isNode === false) {
                console.log(nodeName + " creating in " + parentNodeId + " at " + nodeId);
        ...
        return parentNodeId;  // last node visited, or nodeId into which one should write
    }
