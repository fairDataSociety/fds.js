...
class Multibox {
    ...
    async newRequest(senderAccount, recipientSubdomain, applicationDomain, multiboxAddress, feedLocationHash) {
        ...
        try {
            //console.log("mutlibox newRequest: ", senderAccount, recipientSubdomain, applicationDomain, multiboxAddress, feedLocationHash );
            ...
            // create node at target account (or gracefully fail if it exists) 
            let pathToWriteTo = await this.createPath(senderAccount, nodeName /* applicationDomain*/, multiboxAddress, 0);
    ...
    async getRequest(account, applicationDomain, multiboxAddress) {
        ...
        try {
            ...
            if (isNode === false) { // node does not exists, create one at 
                ...
                let nodeCreated = await this.createPath(account, nodeName /* applicationDomain*/, multiboxAddress, 0);

                console.log("created node for getRequest:" + nodeCreated);
            } else {
    ...
    async getRequests(account, applicationDomain, multiboxAddress) {
        ...
        //console.log("getRequests", account, applicationDomain, multiboxAddress);
        let recepientKvt = await this.getKvt(multiboxAddress, 0); // get 0 KVT of recepient
        let pathToDomain = applicationDomain.substring(0, applicationDomain.length - 1);
    ...
    async traverseMultibox(fromAccount, subdomain)
    {
        ...
        //console.log("multiboxAddress", multiboxAddress);
        let multiboxContract = await this.getMultiboxContract(multiboxAddress);
        //console.log("multiboxContract", multiboxContract);
    ...
    async getMultiboxContract(multiboxAddress) {
        //console.log(multiboxAddress);
        let multiboxContract = await new this.web3.eth.Contract(MultiboxContract.abi, multiboxAddress);
    ...
    async getRoots(multiboxContract) {
        if (multiboxContract.address === undefined) {
            //console.error("contract not initializied", multiboxContract.address);
        }
    ...
    {
        //console.log("creating path " + fullpath);
        let kvtContract = await this.getKvt(multiboxContractAddress, root); 
        let nodes = this.splitPath(fullpath);
        let nodeName = "/";
        ...
        //console.log("creating path nodes " + nodes);
