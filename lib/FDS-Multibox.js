...
class Multibox {
  ...
  async retrieveValues(nodeId) {
    const keyValues = {}
    const keyValuesRaw = await this.rootKvt.getKeyValues(nodeId)
    const keys = keyValuesRaw['keys']
    for (var i = 0; i < keys.length; i++) {
      keyValues[keys[i]] = keyValuesRaw['values'][i]
    }
    return keyValues
  }
  ...
  async retrieveTree(nodeId, values = false, retrievePathNames = false) {
    ...
    const children = await this.rootKvt.getChildren(nodeId)
  ...
  async retrievesDescendants(nodeId, values = false, retrievePathNames = false) {
    ...
    for (var i = 0; i < children.length; i++) {
      const descendants = await this.retrievesDescendants(children[i], values, retrievePathNames)
  ...
  async retrievePathNodes(path) {
    const nodeId = await this.Store.getSwarmDigest(path)
