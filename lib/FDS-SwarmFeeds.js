...
class SwarmFeeds {
    ...
    get(address, topicName, privateKey, responseType = 'text', withAddress = false) {
        ...
        return this.beeClientLib.get(address, topicName).then((hash)=>{
            if(hash === false){
                let e = new Error();
            ...
            return this.sendRequest(`/bytes/${hash}`, 'GET', responseType).then((response)=>{
                if(withAddress){
                    return {
                        value: response,
                        address: hash
    ...
    set(address, topicName, privateKey, data, pin = false) {
        console.log('sx',address, topicName, privateKey, data, toHex(data))
        // let pk = Buffer.from(privateKey.slice(-64), 'hex');
        // let wallet = new UnsafeWallet(pk);
        return this.sendRequest('/bytes', 'POST', 'text', toHex(data), pin).then((hash) => {
            let h = JSON.parse(hash).reference;
            console.log('set1',address, topicName, privateKey, data)
    ...
    sendRequest(url, requestType, responseType, data = false, pinHeader = false) {
        ...
        console.log(url, `${this.swarmNode}${url}`, requestType, responseType, data, headers, )
        return request({
