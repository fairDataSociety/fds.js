...
class SwarmFeeds {
  ...
  set(address, topicName, privateKey, data){
    return this.sendRequest('/bzz-raw:/', 'POST', 'text', utils.toHex(data)).then((hash) => {
      return this.updateResource(topicName, address, privateKey, utils.toHex(hash));
    });
  }
  ...
  getResource(topicName, address) {
    let topic = utils.padRight(utils.toHex(topicName), 64, '0');
  ...
  updateResource(topicName, address, privateKey, state) {
    let data = utils.padLeft(state, 2, '0');
    let topic = utils.padRight(utils.toHex(topicName), 64, '0');        
  ...
  signData(data, privateKey){
    const sigObj = secp256k1.sign(Buffer.from(utils.hexToBytes(data)), Buffer.from(utils.hexToBytes(privateKey)));
  ...
  feedUpdateDigest(metaResponse, data) {
    ...
    let dataBytes = utils.hexToBytes(data);
    ...
    try {
      topicBytes = utils.hexToBytes(metaResponse.feed.topic);
    } catch (err) {
      return undefined;
    }

    try {
      userBytes = utils.hexToBytes(metaResponse.feed.user);
    } catch (err) {
      return undefined;
    }
    ...
    return utils.sha3(utils.bytesToHex(new Uint8Array(buf)));
  }
