...
import {unsafeWallet as UnsafeWallet} from 'swarm-lowlevel';
...
class SwarmFeeds {
    ...
    constructor(config) {
        ...
        this.beeClientLib = new BeeClient(config.beeGateway
     ...
     * @returns {promise} promise which resolves rejects with error based on ajax request outcome.
     */
    get(address, topicName, privateKey, responseType = 'text', withAddress = false) {
        // let pk = Buffer.from(privateKey.slice(-64), 'hex');
        // let wallet = new UnsafeWallet(pk);
        console.log('xxx','get',address, topicName)
        return this.beeClientLib.getB(address, topicName).then((hash)=>{
            if(hash === false){
                let e = new Error();
                e.response = {status: 404, message: "feed does not exist yet"};
                throw e;
            }
            console.log('zzz',hash)
            let hh = bytesToHex(hash).slice(-64);
            console.log('zzzx',hh.slice(-64))
            return this.sendRequest(`/bytes/${hh}`, 'GET', responseType).then((response)=>{
                if(withAddress){
                    return {
                        value: response,
                        address: hash
        ...
        //     return this.sendRequest(`/bzz-raw:/${hash}/`, 'GET', responseType).then((response)=>{
        //         if(withAddress){
        //             return {
        //                 value: response,
        //                 address: hash
     ...
     * @returns {promise} promise which resolves rejects with error based on ajax request outcome.
     */
    geto(address, topicName, responseType = 'text', withAddress = false) {
        return this.getResource(topicName, address).then((hash) => {
            return this.sendRequest(`/bzz-raw:/${hash}/`, 'GET', responseType).then((response)=>{
                if(withAddress){
                    return {
     ...
     * @returns {promise} promise which resolves rejects with error based on ajax request outcome.   
     */
    set(address, topicName, privateKey, data, pin = false) {
        console.log('sx',address, topicName, privateKey, data)
        // let pk = Buffer.from(privateKey.slice(-64), 'hex');
        // let wallet = new UnsafeWallet(pk);
        return this.sendRequest('/bytes', 'POST', 'text', toHex(data), pin).then((hash) => {
            let h = JSON.parse(hash).reference;
            let hb = hexToBytes("0x"+h);
            return this.beeClientLib.setB(address, privateKey, topicName, hb).then(()=>{
                console.log("h",h)
                return h;
            });
        });
     ...
     * @returns {promise} promise which resolves rejects with error based on ajax request outcome.   
     */
    seto(address, topicName, privateKey, data, pin = false) {
        return this.sendRequest('/bzz-raw:/', 'POST', 'text', toHex(data), pin).then((hash) => {
            return this.updateResource(topicName, address, privateKey, toHex(hash)).then(()=>{
                return hash;
            });
        });
    }
