...
class SwarmFeeds {
     ...
     */
    sendRequest(url, requestType, responseType, data = false) {

        if(data){
            data = Buffer.from(Web3Utils.hexToBytes(data));
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            xhttp.onreadystatechange = function () {
                if (this.readyState === 4) {
                    // console.log('resp', url, requestType, responseType, data);                    
                    // console.log(xhttp);
                    if (this.status === 200) {
                        debugger
                        if (responseType === 'arraybuffer') {
            ...
            if (data) {
                ...
                xhttp.send(Buffer.from(Web3Utils.hexToBytes(data)));
            } else {
                xhttp.send();
            }
        });
    ...
    dataToBuffer(data) {
        ...
        // console.log(typeof dataBytes, dataBytes)
