...
class SwarmFeeds {
  ...
  constructor(config) {
    ...
    this.swarmNode = 'https://bee-1.gateway.ethswarm.org';
    this.bee = new Bee('https://bee-1.gateway.ethswarm.org');
  }
  ...
  async get(
    ...
    let r = this.bee.makeFeedReader('sequence', t, address);
  ...
  async set(address, topicName, privateKey, data, pin = false) {
    let t = padLeft(toHex(topicName), 64, '0');
    let w = this.bee.makeFeedWriter('sequence', t, privateKey);
    ...
    let dr = await this.bee.uploadData(POSTAGE_STAMP, p);
    let r = await w.upload(POSTAGE_STAMP, dr);
    return dr;
  ...
  signData(data, privateKey) {
    const sigObj = secp256k1.sign(
      ...
      Buffer.from(hexToBytes(privateKey))
    );
    return `0x${sigObj.signature.toString(
      'hex'
    )}0${sigObj.recovery.toString()}`;
  }
}
