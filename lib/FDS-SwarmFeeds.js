...
class SwarmFeeds {
    ...
    constructor(config) {
        ...
        this.beeClientLib = new BeeClient(config.beeGateway, {timeout: config.httpTimeout});
    }
    ...
    get(address, topicName, privateKey, responseType = 'text', withAddress = false) {
        ...
        return this.beeClientLib.getByAddress(address.slice(-40), topicName).then((hash)=>{
            if(hash === false){
                let e = new Error();
    ...
    set(address, topicName, privateKey, data, pin = false) {
        ...
        return this.sendRequest('/bytes', 'POST', 'text', toHex(data), pin).then((hash) => {
            ...
            return this.beeClientLib.set(wallet, topicName, h).then(()=>{
                return hash;
            });
        });
    }
