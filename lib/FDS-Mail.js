...
class Mail {
     ...
     * @returns {any} success
     */
    send(senderAccount, recipientAccount, file, encryptProgressCallback = console.log, uploadProgressCallback = console.log, progressMessageCallback = console.log) {
        ...
        let recipientSubdomainNameHash = namehash.hash(recipientAccount.subdomain + '.' + this.config.ensConfig.domain);
        let feedLocationHash = this.Mailbox.encodeFeedLocationHash(senderAccount.address, recipientSubdomainNameHash);

        progressMessageCallback(`retrieving public key`);
        return ENS.getPubKey(recipientAccount.subdomain).then((publicKey) => {
            //calculate shared secret
            recipientPublicKey = publicKey;
            let sharedSecret = Crypto.calculateSharedSecret(senderAccount.privateKey, publicKey);
            return sharedSecret;
            ...
            return ENS.getMultihash(recipientAccount.subdomain);
        }).then((hash) => {
            ...
            if ( JSON.stringify(request.values) === JSON.stringify({}) ) {
                ...
                return this.Mailbox.newInboxFeed(senderAccount, senderAccount.subdomain, senderSubdomainNameHash, multiboxAddress, feedLocationHash)
                    .then((request) => {
                        return senderAccount.storeContact(
                            new Contact(
                                {
                                    subdomain: recipientAccount.subdomain,
                                    publicKey: recipientPublicKey,
     ...
     */
    async getSenders(recipientAccount) {
        let ENS = new ENS2(recipientAccount, this.config.ensConfig);
        ...
        let senders = await this.Mailbox.getInboxFeeds(recipientAccount, multiboxAddress);
        let feeds = [];
        for (var i = 0; i < senders.children.length; i++) {
            let senderNamehash = Object.keys(senders.children[0].values)[0];
            let feedLocationHash = senders.children[0].values[senderNamehash];
            feeds.push({
