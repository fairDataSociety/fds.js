...
class Mail {
    ...
    send(senderAccount, recipientSubdomain, file, multiboxPath, encryptProgressCallback = console.log, uploadProgressCallback = console.log, progressMessageCallback = console.log) {
        ...
        //if multihash
            ...
            if ( 
                ...
                return this.Mailbox.createInboxFeed(senderAccount, senderAccount.subdomain, senderSubdomainNameHash, multiboxAddress, feedLocationHash, multiboxPath)
                    .then((request) => {
            ...
            return this.saveMessage(recipientPublicKey, senderAccount, feedLocation.topic, message, multiboxPath, progressMessageCallback);
                ...
                return storedFilehashLocation; // we return where stored file is 
        });
    }
    ...
    saveMessage(recipientPublicKey, senderAccount, topic, message, multiboxPath, progressMessageCallback = console.log) {
        ...
        return this.getAccountMessages(recipientPublicKey, senderAccount, topic, multiboxPath).then((messages) => {
            ...
            console.log("saveMessage", message, messages);
     ...
     * @returns {any} array of messages
     */
    getMessages(type = 'received', account, multiboxPath) {
        switch (type) {
            case 'received':
                return this.getReceivedMessages(account, multiboxPath);
            case 'sent':
                return this.getSentMessages(account, multiboxPath);            
        }
    }
    ...
    getReceivedMessages(recipientAccount, multiboxPath) {
        ...
        return this.getSenders(recipientAccount, multiboxPath).then((senders) => {
            ...
            return Promise.all(feedPromises).then((messageFeeds) => {
                ...
                var m = [].concat.apply([], messageFeeds);
                //console.log(m);
                return m;
            });
        });
    ...
    getMessageFeed(publicKey, address, account, topic) {
        //console.log("getMessageFeed", publicKey, address, account, topic);
        ...
        return this.SwarmStore.retrieveDecryptedValue(topic, address, "0x" + sharedSecret).then((decryptedManifest) => {
                //console.log("getMessageFeed decryptedManifest", decryptedManifest);
            ...
            console.log("get message feed", error);
