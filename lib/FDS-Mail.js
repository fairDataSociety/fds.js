...
class Mail {
     ...
     */
    getMessageFeed(publicKey, address, account, topic, prevMessages = []) {
        ...
        return this.SwarmStore.retrieveDecryptedValue(topic, address, "0x" + sharedSecret).then((decryptedManifest) => {
            ...
            let nextParsed = JSON.parse(decryptedManifest).next;
            ...
            messageObjects = prevMessages + messageObjectsWithHashObjects;
            if(nextParsed){
                return this.getMessageFeed(publicKey, nextParsed, account, topic, messageObjectsWithHashObjects)
            }else{
