...
class Mail {
     ...
     * @returns {any} success
     */
    async send(senderAccount, recipientSubdomain, file, multiboxPath, encryptProgressCallback = console.log, uploadProgressCallback = console.log, progressMessageCallback = console.log) {
        let ENS = new ENS2(senderAccount, this.config.ensConfig);
     ...
     * @returns {any} array of messages
     */
    async getReceivedMessages(recipientAccount, multiboxPath) {
        let ENS = new ENS2(recipientAccount, this.config.ensConfig);
        let sendersAll = await this.getSenders(recipientAccount, multiboxPath);
        let senders = [...new Map(sendersAll.map(item => [item['senderNamehash'], item])).values()]; // distinct contacts to so we don't get duplicated pulls for multiple contacts
        let allMessages = [];

        for (var i = 0; i < senders.length; i++) { 
            try 
            {
                var feedLocation = await this.Mailbox.decodeFeedLocationHash(senders[i].feedLocationHash);
                var publicKey = await ENS.getPubKeyRaw(senders[i].senderNamehash); 
                var messageFeed = await this.getMessageFeed(publicKey, feedLocation.address, recipientAccount, feedLocation.topic, multiboxPath); 
                allMessages = allMessages.concat(messageFeed);
            } catch(err)
            {
                console.log("getReceivedMessages", err);
            }
        }
        return allMessages.sort((a,b)=>{return a.hash.time > b.hash.time ? 1 : -1});
    }    
    /*
    getReceivedMessages(recipientAccount, multiboxPath) {
        ...
        return this.getSenders(recipientAccount, multiboxPath).then((senders) => {
            ...
            return Promise.all(feedPromises).then((messageFeeds) => {
                return [].concat.apply([], messageFeeds).sort((a,b)=>{return a.hash.time > b.hash.time ? 1 : -1});
            });
        });
    }*/
     ...
     * @returns {any} array of messages
     */
    async getSentMessages(recipientAccount, multiboxPath) {
        let allMessages = [];
        var contactsAll = await recipientAccount.getContacts(); 
        let contacts = [...new Map(contactsAll.map(item => [item['subdomain'], item])).values()]; // distinct contacts to so we don't get duplicated pulls for multiple contacts
        //console.log("dst contacts", contacts);

        for (var i = 0; i < contacts.length; i++) {
            try 
            {
            let feedLocation = await this.Mailbox.decodeFeedLocationHash(contacts[i].feedLocationHash);
            let contactMessages = await this.getMessageFeed(contacts[i].publicKey, feedLocation.address, recipientAccount, feedLocation.topic, multiboxPath);
            allMessages = allMessages.concat(contactMessages);
            } catch(err)
            {
                console.log("getSentMessages", err);
            }
        }
        return allMessages.sort((a,b)=>{return a.hash.time > b.hash.time ? 1 : -1});
    }
/*
     ...
     */
    async getMessageFeed(publicKey, address, account, topic, mulitboxPath) {
        try 
        {
            let sharedSecret = Crypto.calculateSharedSecret(account.privateKey, publicKey);
            var decryptedManifest = await this.SwarmStore.retrieveDecryptedValue(topic, address, "0x" + sharedSecret); 
        ...
        } catch(error)
        {
            if (error.response.status !== 404) {
                console.log("getMessageFeed", error)
            } 
            return [];
        }
    }    
    /*
    getMessageFeed(publicKey, address, account, topic, mulitboxPath) {
        ...
        return this.SwarmStore.retrieveDecryptedValue(topic, address, "0x" + sharedSecret).then((decryptedManifest) => {
            let messageParsed = JSON.parse(decryptedManifest).messages;
            //console.log("parsed messageParsed", decryptedManifest);
            let messageObjects = messageParsed.map((m) => new Message(m, this.config, account));
            console.log("parsed messageObjects", messageObjects);
            let messageObjectsWithHashObjects = messageObjects.map((m) => {
                m.hash = new Hash(m.hash, this.config);
                return m;
            });
