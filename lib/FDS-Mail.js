...
class Mail {
    ...
    lookupContact(senderAccount, recipientSubdomain, encryptProgressCallback = console.log, uploadProgressCallback = console.log, progressMessageCallback = console.log)
    {
        ...
        console.log('X',senderAccount, recipientSubdomain, encryptProgressCallback = console.log, uploadProgressCallback = console.log, progressMessageCallback = console.log)
     ...
     * @returns {any} array of messages
     */
    getMessages(type = 'received', account, path = '/shared/mail/shared') {
        switch (type) {
            case 'received':
    ...
    async getSenders(recipientAccount, multiboxPath) {
        ...
        let feeds = [];
        if(senders.children !== undefined){
            for (var i = 0; i < senders.children.length; i++) {
                let senderNamehash = Object.keys(senders.children[i].values)[0];
                let feedLocationHash = senders.children[i].values[senderNamehash];
                feeds.push({
