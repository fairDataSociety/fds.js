...
class Mail {
  ...
  lookupContact(senderAccount, recipientSubdomain) {
    ...
    let recipientSubdomainNameHash = hash(
      recipientSubdomain + '.' + this.config.ensConfig.domain
    );
    let feedLocationHash = this.Mailbox.encodeFeedLocationHash(
      senderAccount.address,
      recipientSubdomainNameHash
    );
  ...
  send(
    ...
    let senderSubdomainNameHash = hash(
      senderAccount.subdomain + '.' + this.config.ensConfig.domain
    );
    let recipientSubdomainNameHash = hash(
      recipientSubdomain + '.' + this.config.ensConfig.domain
    );
    ...
    return ENS.getPubKey(recipientSubdomain)
      .then((publicKey) => {
        ...
        let sharedSecret = Crypto.calculateSharedSecret(
      ...
      .then((hash) => {
        ...
        progressMessageCallback(`retrieving connection`);
        if (multiboxAddress !== '0x') {
          return this.Mailbox.getInboxFeed(
            senderAccount,
      ...
      .then((request) => {
        ...
        if (
          ...
          progressMessageCallback(
            `creating connection ${senderAccount.subdomain}->${recipientSubdomain}`
          );
      ...
      .then((storedFilehash) => {
        ...
        let feedLocation =
          this.Mailbox.decodeFeedLocationHash(feedLocationHash);
        message = new Message(
          {
  ...
  saveMessage(
    ...
    return this.getAccountMessages(
      recipientPublicKey,
      senderAccount,
      topic,
      multiboxPath
      ...
      for (var i = senders.length - 1; i >= 0; i--) {
        feedPromises.push(
          new Promise((resolve, reject) => {
            var feedLocation = this.Mailbox.decodeFeedLocationHash(
              senders[i].feedLocationHash
            );
            return ENS.getPubKeyRaw(senders[i].senderNamehash).then(
              (publicKey) => {
                return this.getMessageFeed(
                  publicKey,
  ...
  getSentMessages(recipientAccount, multiboxPath) {
    return recipientAccount.getContacts().then((contacts) => {
      ...
      for (var i = contacts.length - 1; i >= 0; i--) {
        feedPromises.push(
          new Promise((resolve, reject) => {
            let feedLocation = this.Mailbox.decodeFeedLocationHash(
              contacts[i].feedLocationHash
            );
            return this.getMessageFeed(
  ...
  getMessageFeed(publicKey, address, account, topic) {
    ...
    return this.SwarmStore.retrieveDecryptedValue(
      topic,
      address,
      '0x' + sharedSecret
    )
      .then((decryptedManifest) => {
        ...
        let messageObjects = messageParsed.map(
          (m) => new Message(m, this.config, account)
        );
  ...
  async getSenders(recipientAccount, multiboxPath) {
    ...
    if (multiboxAddress === null) {
      throw new Error(
        `couldn't find mailbox for ${recipientAccount.subdomain}`
      );
    }
    let senderTree = await this.Mailbox.getInboxFeeds(
      recipientAccount,
      multiboxAddress,
      multiboxPath
    );
