            ...
            if ( JSON.stringify(request.values) === JSON.stringify({}) ) {
                ...
                return this.Mailbox.newInboxFeed(senderAccount, senderAccount.subdomain, senderSubdomainNameHash, multiboxAddress, feedLocationHash)
                    .then((request) => {
                        ...
                        console.log('init', recipientPublicKey, senderAccount, flh.topic)
            ...
            for (var i = senders.length - 1; i >= 0; i--) {
                feedPromises.push(new Promise((resolve, reject) => {
                    ...
                    return ENS.getPubKeyRaw(senders[i].senderNamehash).then((publicKey) => {
                        console.log('r', publicKey, feedLocation.address, recipientAccount, feedLocation.topic)
