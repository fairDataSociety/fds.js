...
class SwarmStore {
    ...
    storeEncryptedValue(key, value, storerAccount, password) {
        ...
        // console.log('ivs', password, iv, storerAccount.address, value)
        return Crypto.encryptString(value, password, iv).then((encryptedString) => {
            let ivAndEncryptedString = iv + encryptedString;
            // console.log('store enc', storerAccount.address, key, storerAccount.privateKey, ivAndEncryptedString);
    ...
    retrieveDecryptedValue(key, address, password) {
        // console.log('retrieve', key, address, password);
        return this.SF.get(address, key, 'arraybuffer').then((encryptedBuffer) => {
            // debugger
            let ivAndEncryptedString = encryptedBuffer.toString();
            let iv = ivAndEncryptedString.substring(0, 16);
            // console.log('ivr', password, iv, address, ivAndEncryptedString, encryptedBuffer )            
            let encryptedString = ivAndEncryptedString.substring(16);
