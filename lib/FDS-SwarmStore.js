...
class SwarmStore {
    ...
    updateStoredMeta(hash, updateMeta, storerAccount){
        return this.getStoredManifest(storerAccount).then((manifest)=>{
            for (var i = 0; i < manifest.storedFiles.length; i++) {
                if(manifest.storedFiles[i].address === hash.address){
                    let meta;
    ...
    updateStoredManifest(manifest, storerAccount){
        return this.storeEncryptedValue('stored-1.0', JSON.stringify(manifest), storerAccount, storerAccount.privateKey).then((response)=>{
          return JSON.parse(response);
        });
    }    
     ...
     */
    storeFile(storerAccount, file, encryptProgressCallback, uploadProgressCallback, progressMessageCallback, metadata) {
        return this.Store.storeEncryptedFile(file, storerAccount.privateKey, encryptProgressCallback, uploadProgressCallback, progressMessageCallback, true, metadata).then((hash) => {
            return this.getStored('all', storerAccount).then((storedFiles) => {
                ...
                if(isStored === false){
                    ...
                    return this.storeEncryptedValue('stored-1.0', JSON.stringify({ storedFiles: storedFiles }), storerAccount, storerAccount.privateKey).then(()=>{
                        return hash;
                    });
                }else{
                    return false;
