...
class SwarmStore {
        ...
        return this.Store.storeEncryptedFile(file, storerAccount.privateKey, encryptProgressCallback, uploadProgressCallback, progressMessageCallback, true, metadata).then((hash) => {
            return this.getStored('all', storerAccount).then((response) => {
                ...
                if(isStored === false){
                    ...
                    return this.storeEncryptedValue('stored-1.0', JSON.stringify({ storedFiles: storedFiles }), storerAccount, storerAccount.privateKey, pin).then((storedManifestAddress)=>{
                        if(showManifestAddresses === true){
                            return {
                                storedFile: hash,
                                storedManifestAddress: JSON.parse(storedManifestAddress).reference,
    ...
    retrieveDecryptedValue(key, address, password, withAddress = false) {
        console.log("yxxxx", key, address, password, withAddress)
