...
class SwarmStore {
  ...
  getStored(query, storerAccount) {
    return this.retrieveDecryptedValue(
      'stored-1.0',
      storerAccount.address,
      storerAccount.privateKey,
      true
    )
      .then((response) => {
  ...
  retrieveDecryptedValue(key, address, password, withAddress = false) {
    if (withAddress === false) {
      return this.SF.get(address, key, password, 'arraybuffer').then(
        (encryptedBuffer) => {
          let ivAndEncryptedString = new TextDecoder()
            .decode(encryptedBuffer)
            .replace('0x', '');
      ...
      return this.SF.get(address, key, password, 'arraybuffer', true).then(
        (response) => {
