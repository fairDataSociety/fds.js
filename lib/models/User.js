...
class User {
   ...
   * @param {any} encryptionCallback callback to call
   * @param {any} uploadCallback callback to call
   * @param {any} progressMessageCallback callback to call
   * @returns {any} result
   */
  async send(recipientSubdomain, file, multiboxPath, encryptionCallback = console.log, uploadCallback = console.log, progressMessageCallback = console.log){
    return await this.Mail.send(this, recipientSubdomain, file, multiboxPath, encryptionCallback, uploadCallback, progressMessageCallback); 
  }
  ...
  */
  async payAddress(recipientAddress, amount, gas = 15000000, transactionCallback = console.log, transactionSignedCallback = console.log) {
       return await this.Tx.pay(this, recipientAddress, amount, gas, transactionCallback, transactionSignedCallback); 
  }
  ...
  */
  async pay(recipientSubdomain, amount, gas = 15000000, transactionCallback = console.log, transactionSignedCallback = console.log) {
    ...
    //debugger;
    return await this.payAddress(recipientAddress, amount, gas, transactionCallback, transactionSignedCallback);
  }
  ...
  messages(query = 'received', multiboxPath){
     ...
     */
  storeValue(key, value, pin = false){
   ...
   */
  store(file, encryptionCallback = console.log, uploadCallback = console.log, progressMessageCallback = console.log, metadata = {}, showManifestAddresses = false, pin = false){
  ...
  async getAddressOf(subdomain) {
    let contact = await this.lookupContact(subdomain);
