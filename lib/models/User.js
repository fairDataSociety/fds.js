...
class User {
  ...
  deployContract(abi, bytecode, args = [], nonce, gas = 6000000) {
  ...
  send(
    ...
    return this.Mail.sendToAccount({
      sender: this,
      recipient: recipientSubdomain,
      file,
      multiboxPath,
  ...
  store(
    file,
    encryptionCallback = console.log,
    uploadCallback = console.log,
    progressMessageCallback = console.log,
    metadata = {},
    showManifestAddresses = false,
    pin = false,
  ) {
    return this.SwarmStore.storeFile(
      this,
      file,
      encryptionCallback,
      uploadCallback,
  ...
  getBackupFile() {
    return new File([JSON.stringify(this.wallet)], `fds-wallet-${this.subdomain}-backup.json`, {
