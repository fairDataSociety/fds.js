...
class Mailbox {
    ...
    async newInboxFeed(senderAccount, subdomain, senderSubdomainNamehash, multiboxAddress, feedLocationHash, multiboxPath) {
    ...
    async getInboxFeed(senderAccount, subdomain, multiboxAddress, path = '/shared/mail/shared'){
        let multibox = await new Multibox(senderAccount, this.config).at(multiboxAddress);
        return multibox.get( path = '/shared/chattie/'+subdomain);
    }
