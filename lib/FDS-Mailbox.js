...
class Mailbox {
    ...
    async newInboxFeed(senderAccount, subdomain, senderSubdomainNamehash, multiboxAddress, feedLocationHash, applicationDomain = 'shared') {
        let multibox = await new Multibox(senderAccount, this.config).at(multiboxAddress);
        return multibox.set('/shared/mail/'+applicationDomain+'/'+senderAccount.subdomain, senderSubdomainNamehash, feedLocationHash);
    }

    async getInboxFeed(senderAccount, subdomain, multiboxAddress, path = '/shared/mail/shared'){
        let multibox = await new Multibox(senderAccount, this.config).at(multiboxAddress);
        return multibox.get( path = '/shared/mail/shared/'+subdomain);
    }

    async getInboxFeeds(account, multiboxAddress, path = '/shared/mail/shared'){
        let multibox = await new Multibox(account, this.config).at(multiboxAddress);
        console.log('p',path)
    ...
    //  */
    // getRequest(namehash, mailboxAddress) {
    //     let recipientMailboxContract = new this.web3.eth.Contract(MailboxContract.abi, mailboxAddress);
