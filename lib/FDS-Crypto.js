...
class Crypto {
  ...
  privateToPublicKey(privateKey) {
    const secp256k1 = new ec('secp256k1') // eslint-disable-line
  ...
  encryptString(string, password, iv) {
    const encryptedBuffer = this.encryptBuffer(Buffer.from(string, 'utf8'), password, iv)
  ...
  encryptBuffer(buffer, password, iv) {
    let cipher = createCipheriv('aes-256-ctr', Buffer.from(password.substring(2), 'hex'), iv)
    let crypted = Buffer.concat([cipher.update(buffer), cipher.final()])
    return crypted
  }

  decryptBuffer(buffer, password, iv) {
    var decipher = createDecipheriv('aes-256-ctr', Buffer.from(password.substring(2), 'hex'), iv)
