...
class FDSENS {
    ...
    constructor(config = {}) {
        ...
        this.web3 = new Web3(config.ethGateway);

        var provider = new Web3.providers.HttpProvider(config.ethGateway);
        // this.ens = new ENS(provider, config.ensConfig.registryAddress);
    ...
    registerSubdomain(subdomain, wallet, nonce) {
        ...
        return this.web3.eth.accounts.signTransaction(tx, privateKey).then((signed) => {
            return this.web3.eth.sendSignedTransaction(signed.rawTransaction)
                // .once('transactionHash', function(confNumber, receipt){ 
                //     console.log('1', 'registerSubdomain')
                //     return receipt; 
                // });
        });
    }
